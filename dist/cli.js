...
async function main() {
    ...
    try {
        ...
        if (result.registered.length > 0 && result.claudeSkillsDir) {
            console.log(`[natskill] Registered ${result.registered.length} skills for Claude Code in ${result.claudeSkillsDir}.`);
        }
        if (result.registeredCodex.length > 0 && result.codexSkillsDir) {
            console.log(`[natskill] Registered ${result.registeredCodex.length} skills for Codex in ${result.codexSkillsDir}.`);
        }
        if (result.pointerFiles.length > 0) {
            console.log(`[natskill] Wrote orchestrator pointer to ${result.pointerFiles.join(", ")}.`);
        }
